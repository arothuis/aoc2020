...
const plot = (f, end, coords = [], x = 0) => 
    end === 0 ? coords : plot(f, end - 1, [...coords, [x, f(x)]], x + 1);

const countTreesOnPath = (land, coords) => 
    getPath(land, coords).filter(x => x === "#").length;

const solveA = path => {
    const land = parseLand(linesFromFile(path));
    const coords= plot(x => 3 * x, land.length);
    return countTreesOnPath(land, coords);
};
const solveB = path => {
    const land = parseLand(linesFromFile(path));
    return  [x => x, x => 3 * x, x => 5 * x, x => 7 * x, x => 0.5 * x]
